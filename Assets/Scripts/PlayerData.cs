...
{
    [Header("PlayerData")] public float health = 100f;
    ...
    private void Update()
    {
        if (staminaRegenDelayTimer <= staminaRegenDelay) staminaRegenDelayTimer += Time.deltaTime;
        if (staminaRegenDelayTimer > staminaRegenDelay) staminaRegenDelayTimer = staminaRegenDelay;
        ...
        if (healthRegenDelayTimer <= healthRegenDelay) healthRegenDelayTimer -= Time.deltaTime;
        if (healthRegenDelayTimer > healthRegenDelay) healthRegenDelayTimer = healthRegenDelay;
